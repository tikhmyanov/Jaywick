...
get_header(); ?>
    ...
    <section class="content-module portfolio clearfix type-<?php echo $type; ?>" data-cols="<?php echo $cols; ?>">
		...
		while ( $all_posts->have_posts() ) : $all_posts->the_post();
			...
			$l_class = '';
			$l_image = get_post_meta( $post->ID, 'lobo_folio_l_img', true );
			$l_iframe = get_post_meta( $post->ID, 'lobo_folio_l_iframe', true );
		...
		<?php if ( $type == 'full' ) :  ?>
			...
			<div class="module image-module <?php lobo_categories( $post->ID, 'portfolio_category', ' ', 'slug' ); ?> ifpt" data-size="4-2" data-bgimage="<?php echo $old_img[0]; ?>">
				...
				<a class="view-item-btn pfl<?php echo $l_class; ?>" title="<?php the_title(); ?>" href="<?php echo $link; ?>"<?php echo ( $target != '' ? ' target="' . $target . '"' : '' ); ?>><?php _e( 'View', 'lobo' ); ?><i class="icon-fontawesome-webfont-7"></i></a>
			</div>
		...
		<?php else : 
			...
			<a class="prtfl-item <?php lobo_categories( $post->ID, 'portfolio_category', ' ', 'slug' ); ?> ifpt pfl<?php echo $l_class; ?>" title="<?php the_title(); ?>" href="<?php echo $link; ?>"<?php echo ( $target != '' ? ' target="' . $target . '"' : '' ); ?>>
